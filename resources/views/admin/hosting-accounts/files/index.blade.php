...
<x-admin-layout>
    ...
    <div class="file-toolbar-path d-flex align-items-center gap-2 mb-3 flex-wrap">
        ...
        @if ($parentPath !== null)
            <a href="{{ route('admin.hosting-accounts.files.index', [$account, 'path' => $parentPath, 'root' => $root]) }}"
                        ...
                        @forelse ($entries as $entry)
                            @php
                                ...
                                $navigateUrl = match ($kind) {
                                    'directory' => route('admin.hosting-accounts.files.index', [$account, 'path' => $entryPath, 'root' => $root]),
                                    'text' => route('admin.hosting-accounts.files.edit', [$account, 'path' => $entryPath, 'root' => $root]),
                                    default => null,
                                };
                                $downloadUrl = $entry['type'] === 'file'
                                    ? route('admin.hosting-accounts.files.download', [$account, 'path' => $entryPath, 'root' => $root])
                                    : null;
                            @endphp
