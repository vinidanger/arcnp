<x-admin-layout>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('admin.hosting-accounts.ssh.keys.store', $account) }}">
                @csrf
                <div class="row g-2">
                    <div class="col-md-4">
                        <x-input-label for="name" value="{{ __('Nome') }}" class="small mb-1" />
                        <input type="text" id="name" name="name" class="form-control form-control-sm" placeholder="notebook do fulano" required>
                    </div>
                    <div class="col-md-8">
                ...
                <tbody>
                    @forelse ($account->sshKeys as $key)
                        <tr>
                            ...
                            <td class="text-end">
                                <form method="POST" action="{{ route('admin.hosting-accounts.ssh.keys.destroy', [$account, $key]) }}"
