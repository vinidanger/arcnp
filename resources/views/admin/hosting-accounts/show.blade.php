                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-end">
                                        ...
                                        <form method="POST" action="{{ route('admin.hosting-accounts.cache.update', $account) }}" class="d-inline"
                                              onsubmit="return confirm('{{ __('Isso reescreve a configuração desse domínio no servidor — o site fica fora do ar por um instante. Continuar?') }}')">
                                            @csrf
                                        ...
                                        @if ($account->cache_enabled)
                                            <form method="POST" action="{{ route('admin.hosting-accounts.cache.purge', $account) }}" class="d-inline">
                                                @csrf
                                ...
                                @foreach ($account->domains as $domain)
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                ...
                                                <form method="POST" action="{{ route('admin.hosting-accounts.domains.cache.update', [$account, $domain]) }}" class="d-inline"
                                                      onsubmit="return confirm('{{ __('Isso reescreve a configuração desse domínio no servidor — o site fica fora do ar por um instante. Continuar?') }}')">
                                                    @csrf
                                                ...
                                                @if ($domain->cache_enabled)
                                                    <form method="POST" action="{{ route('admin.hosting-accounts.domains.cache.purge', [$account, $domain]) }}" class="d-inline">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-outline-secondary">{{ __('Limpar cache') }}</button>
                                                    </form>
                                                @endif
