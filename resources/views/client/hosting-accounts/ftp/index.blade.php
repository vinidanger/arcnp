<x-client-layout>
    ...
    @if (session('plain_ftp_password'))
        <div class="alert alert-warning">
            ...
            <pre class="mb-0 mt-2 bg-white p-2 rounded border small">{{ session('plain_ftp_password') }}</pre>
        </div>
    @endif
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('client.hosting-accounts.ftp.store', $account) }}" class="row g-2 align-items-end">
                @csrf
                <div class="col-auto">
            ...
            <div class="form-text mt-2">{{ __('Pasta relativa à sua conta (ex: public_html ou public_html/blog) — a pasta precisa já existir. Deixe vazio pra acesso à conta inteira.') }}</div>
                    ...
                    @forelse ($account->ftpAccounts as $ftpAccount)
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                <form method="POST" action="{{ route('client.hosting-accounts.ftp.destroy', [$account, $ftpAccount]) }}"
    ...
    @foreach ($account->ftpAccounts as $ftpAccount)
        <x-modal name="ftp-password-modal-{{ $ftpAccount->id }}" maxWidth="sm">
            <form method="POST" action="{{ route('client.hosting-accounts.ftp.password.update', [$account, $ftpAccount]) }}">
                @csrf
                @method('PUT')
