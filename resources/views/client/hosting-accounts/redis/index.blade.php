    ...
    <div class="card mb-3">
        <div class="card-body">
            <p class="small text-secondary">
                {{ __('Instância compartilhada do servidor, isolada por usuário — sua conta só enxerga as próprias chaves. Use as credenciais abaixo no plugin de cache de objeto do seu site (ex. "Redis Object Cache" pro WordPress).') }}
            </p>
            ...
            <form method="POST" action="{{ route('client.hosting-accounts.redis.regenerate', $account) }}"
                  @if ($account->redis_username) onsubmit="return confirm('{{ __('Isso invalida a senha atual — qualquer plugin já configurado com ela para de funcionar até você atualizar. Continuar?') }}')" @endif>
                @csrf
