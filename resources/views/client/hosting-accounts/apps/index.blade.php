<x-client-layout>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('client.hosting-accounts.apps.store', $account) }}" class="row g-2 align-items-end">
                @csrf
                <div class="col-auto">
                ...
                <tbody>
                    @forelse ($account->hostedApps as $app)
                        <tr>
                            ...
                            <td class="text-end">
                                <form method="POST" action="{{ route('client.hosting-accounts.apps.restart', [$account, $app]) }}" class="d-inline-block">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-outline-secondary">{{ __('Reiniciar') }}</button>
                                </form>
                                <form method="POST" action="{{ route('client.hosting-accounts.apps.destroy', [$account, $app]) }}"
