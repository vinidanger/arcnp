...
class HostingAccountProvisioningService
{
     ...
     * subdomínio novo (árvore própria, domains/{staging}/public_html,
     * nunca aninhado dentro do public_html de produção), clona os
     * arquivos do domínio PRINCIPAL nele, e — se a conta tiver banco —
     * clona o PRIMEIRO banco também. Se a instalação no domínio
     * principal for WordPress, reescreve só as 4 constantes de banco no
     * wp-config.php da cópia (senão a cópia continuaria escrevendo no
     * banco de PRODUÇÃO). Qualquer falha no meio desfaz o que já foi
     * criado (mesmo padrão de compensação do resto do serviço).
     *
     * Clone de arquivo+wp-config é síncrono (bloqueia até terminar);
     * database.clone é ASSÍNCRONO (dump+import pode demorar num banco
     * grande) — como as credenciais do banco novo já existem antes do
     * dump/import terminar, o wp-config já nasce apontando pro lugar
     * certo, só o CONTEÚDO do banco chega um pouco depois em segundo
     * plano. Falha do clone de dados (só descoberta depois, via
     * callback) é tratada em AgentWebhookController::applyDatabaseCloneResult().
     */
    public function cloneToStaging(HostingAccount $account, string $subdomainLabel): Domain
    {
        if ($account->domains()->count() >= $account->plan->max_addon_domains) {
            throw new RuntimeException('Limite de domínios adicionais do plano atingido.');
        }

        $stagingDomainName = "{$subdomainLabel}.{$account->primary_domain}";

        $domain = $this->addDomain($account, $stagingDomainName, 'addon', 'outside_public_html');

        if ($domain->status !== 'active') {
            throw new RuntimeException($domain->last_error ?? 'Falha ao criar o domínio de staging.');
        }

        $domain->update(['is_staging' => true]);

        $stagingDatabase = null;

        try {
            $this->runStep($account->server, 'files.clone_site', [
                'username' => $account->linux_username,
                'source_domain' => $account->primary_domain,
                'source_location' => null,
                'source_subdir' => null,
                'source_public_path' => $account->public_path,
                'dest_domain' => $stagingDomainName,
            ]);

            $sourceDatabase = $account->databases()->first();

            if ($sourceDatabase) {
                $stagingDatabase = $this->provisionDatabase($account, $subdomainLabel);
                $stagingDatabase->update(['is_staging' => true]);

                $job = $this->client->dispatch($account->server, 'database.clone', [
                    'source_database' => $sourceDatabase->db_name,
                    'dest_database' => $stagingDatabase->db_name,
                    'staging_domain_id' => $domain->id,
                ]);

                if ($job->status === 'failed') {
                    throw new RuntimeException($job->error ?? 'Falha ao disparar o clone do banco de dados.');
                }

                $wordpressInstallation = $account->appInstallations()
                    ->where('domain', $account->primary_domain)
                    ->where('catalog_slug', 'wordpress')
                    ->first();

                if ($wordpressInstallation) {
            ...
            if ($stagingDatabase) {
                $this->deleteDatabase($stagingDatabase);
            }
